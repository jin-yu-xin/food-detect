...
for class_name in os.listdir(LABLED_DIR):
    ...
    # 随机生成训练集和验证集
    num_labeled_list = int(len(labeled_list))
    labeled_list_index = list(range(num_labeled_list))
    random.shuffle(labeled_list_index)
    num_train = int(len(labeled_list) * TRAIN_PERCENT)
    for i in labeled_list_index[:num_train]:
        labeled_dict['train'].append(labeled_list[i])
    for i in labeled_list_index[num_train:]:
        labeled_dict['val'].append(labeled_list[i])
